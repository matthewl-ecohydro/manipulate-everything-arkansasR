###
###
### 11/18/2025 Adding in Monthly Stats with RFD01 and Arkrocco stations
###
###
...
arkrocco <- read.csv(file = "C:/Users/19139/Desktop/NSF_Hypoth2_repeat/To_GitHub/historic_timeseries/1992_now_Arkrocco_daily_ts.csv",header = TRUE)
rfd01 <- read.csv(file = "C:/Users/19139/Desktop/NSF_Hypoth2_repeat/To_GitHub/historic_timeseries/1992_now_RFD01_daily_ts.csv",header = TRUE)

# Compute monthly sums of Precip and ET

rfd01$datez <- as.Date(as.POSIXct(rfd01$Date, format = "%m/%d/%Y"))
arkrocco$datez <- as.Date(as.POSIXct(arkrocco$Date.Time, format = "%m/%d/%Y %H:%M"))

rfd01[rfd01 == -999.00] <- NA


q_avgs <- arkrocco %>%
  mutate(year_month = format(datez, "%Y-%m")) %>%
  group_by(year_month) %>%
  summarise(q_avgs = mean(DISCHRG.Value, na.rm=TRUE)) %>%
  ungroup()

q_avgs$datez <- as.Date(paste0(q_avgs$year_month, "-01"), format = "%Y-%m-%d")

q_avgs$month_numeric <- format(q_avgs$datez, "%m")

q_avgs$q_avgs_cms <- q_avgs$q_avgs * 0.0283168


# Calculate the 5th, 50th (median), and 95th percentiles for each month
q_mo_prcntls <- q_avgs %>%
  group_by(month_numeric) %>%
  summarize(
    Median = quantile(q_avgs_cms, 0.50),
    p95 = quantile(q_avgs_cms, 0.95)
  )

print(q_mo_prcntls)

q_mo <- as.data.frame(matrix(c(q_avgs$q_avgs[c(280:387)]* 0.0283168), ncol = 9, byrow = TRUE))

colnames(q_mo) <- c("y2016","y2017","y2018","y2019","y2020","y2021","y2022","y2023","y2024")

q_mo_prcntls <- cbind.data.frame(q_mo_prcntls, q_mo)

q_mo_prcntls$month_numeric <- as.numeric(q_mo_prcntls$month_numeric)

q_mo_prcntls_long <- q_mo_prcntls %>%
  pivot_longer(
    cols = -month_numeric, # This specifies to pivot all columns
    names_to = "Statistic", # New column to store original column names
    values_to = "Q_avg" # New column to store the values
  )


q_mo_prcntls_long$month_numeric <- as.factor(q_mo_prcntls_long$month_numeric)

q_mo_prcntls_long$Statistic <- as.factor(q_mo_prcntls_long$Statistic)

q_mo_prcntls_long$group = factor(rep(c("A_dashed", "B_dashed", "C_solid", "D_solid","E_solid",
                                       "F_solid", "G_solid","H_solid","I_solid", "J_solid","K_solid"), times = 12))


# Extract line type and color information from the group variable
q_mo_prcntls_long$line_type_group <- ifelse(grepl("dashed", q_mo_prcntls_long$group), "dashed", "solid")
q_mo_prcntls_long$color_group <- gsub("_.*", "", q_mo_prcntls_long$group) # Extract A, B, C, D

# Get the last point of each line for labeling
label_data <- q_mo_prcntls_long %>%
  group_by(Statistic) %>%
  filter(Q_avg == max(Q_avg))

# Create the ggplot
plot_a <- ggplot(q_mo_prcntls_long, aes(x = month_numeric, y=Q_avg, group = Statistic, color = Statistic, linetype = line_type_group)) +
  geom_line() + scale_color_viridis_d() +
  scale_linetype_manual(values = c("solid" = "solid", "dashed" = "dashed")) + # Manually define linetypes
  labs(
    title = "(a)",
    x = "Time (mo)",
    y = "Avg. Monthly Discharge (cms)",
    linetype = "Line Style"
  ) +
  geom_text_repel(data = label_data, aes(label = Statistic),
                  nudge_x = 0.75, # Adjust label position
                  direction = "y", # Allow movement in y-direction
                  box.padding = 0.5) + theme(legend.position = "none")
...
prec_sums <- rfd01 %>%
  mutate(year_month = format(datez, "%Y-%m")) %>%
  group_by(year_month) %>%
  summarise(prec_tot = sum(Liquid.Precip, na.rm=TRUE)) %>%
  ungroup()

et_sums <- rfd01 %>%
  mutate(year_month = format(datez, "%Y-%m")) %>%
  group_by(year_month) %>%
  summarise(et_tot = sum(ASCE.ETr, na.rm=TRUE)) %>%
  ungroup()


pet_sums <- as.data.frame(prec_sums$prec_tot - et_sums$et_tot)
colnames(pet_sums) <- c("P_ET")
pet_sums$year_month <- et_sums$year_month

pet_sums$datez <- as.Date(paste0(pet_sums$year_month, "-01"), format = "%Y-%m-%d")

pet_sums$month_numeric <- format(pet_sums$datez, "%m")

# Calculate the 5th, 50th (median), and 95th percentiles for each month
pet_mo_prcntls <- pet_sums %>%
  group_by(month_numeric) %>%
  summarize(
    Median = quantile(P_ET, 0.50),
    p95 = quantile(P_ET, 0.95)
  )



pet_mo <- as.data.frame(matrix(c(pet_sums$P_ET[c(284:391)]), ncol = 9, byrow = TRUE))

colnames(pet_mo) <- c("y2016","y2017","y2018","y2019","y2020","y2021","y2022","y2023","y2024")

pet_mo_prcntls <- cbind.data.frame(pet_mo_prcntls, pet_mo)

pet_mo_prcntls$month_numeric <- as.numeric(pet_mo_prcntls$month_numeric)

pet_mo_prcntls_long <- pet_mo_prcntls %>%
  pivot_longer(
    cols = -month_numeric, # This specifies to pivot all columns
    names_to = "Statistic", # New column to store original column names
    values_to = "P_ET" # New column to store the values
  )




pet_mo_prcntls_long$month_numeric <- as.factor(pet_mo_prcntls_long$month_numeric)

pet_mo_prcntls_long$Statistic <- as.factor(pet_mo_prcntls_long$Statistic)

pet_mo_prcntls_long$group = factor(rep(c("A_dashed", "B_dashed", "C_solid", "D_solid","E_solid",
                                       "F_solid", "G_solid","H_solid","I_solid", "J_solid","K_solid"), times = 12))


# Extract line type and color information from the group variable
pet_mo_prcntls_long$line_type_group <- ifelse(grepl("dashed", pet_mo_prcntls_long$group), "dashed", "solid")
pet_mo_prcntls_long$color_group <- gsub("_.*", "", pet_mo_prcntls_long$group) # Extract A, B, C, D

# Get the last point of each line for labeling
label_data <- pet_mo_prcntls_long %>%
  group_by(Statistic) %>%
  filter(P_ET == min(P_ET))

# Create the ggplot
plot_b <- ggplot(pet_mo_prcntls_long, aes(x = month_numeric, y=P_ET, group = Statistic, color = Statistic, linetype = line_type_group)) +
  geom_line() + scale_color_viridis_d() +
  scale_linetype_manual(values = c("solid" = "solid", "dashed" = "dashed")) + # Manually define linetypes
  labs(
    title = "(b)",
    x = "Time (mo)",
    y = "Montly Total P-ET (mm)") +
  geom_text_repel(data = label_data, aes(label = Statistic),
                  nudge_x = 0.75, # Adjust label position
                  direction = "y", # Allow movement in y-direction
                  box.padding = 0.5) + theme(legend.position = "none")
...
# Do Water Year Bar Plots for Precip, Q, and ET WY annual totals...
...
rfd01 <- rfd01 %>%
  mutate(WaterYear = water_year(datez))

prec_sums_yr <- rfd01 %>%
  group_by(WaterYear) %>%
  summarize(TotalPrecipitation = sum(Liquid.Precip, na.rm=TRUE),
            .groups = 'drop') # Drop grouping for cleaner output

# Keep only WY 2017 going forward...
prec_sums_yr <- as.data.frame(prec_sums_yr[c(26:33),])

prec_sums_yr$WY <- as.factor(prec_sums_yr$WaterYear)

# wy_p <- ggplot(prec_sums_yr, aes(x = WY, y = TotalPrecipitation, fill = WY)) +
#   geom_bar(stat = "identity") + scale_fill_viridis_d() + 
#     labs(x="Water Year, 2017-2024", y = "Precip. Total (mm)", fill = "Water Year")
...
et_sums_yr <- rfd01 %>%
  group_by(WaterYear) %>%
  summarize(TotalET = sum(ASCE.ETr, na.rm=TRUE),
            .groups = 'drop') # Drop grouping for cleaner output

# Keep only WY 2017 going forward...
et_sums_yr <- as.data.frame(et_sums_yr[c(26:33),])

et_sums_yr$WY <- as.factor(et_sums_yr$WaterYear)

# wy_et <- ggplot(et_sums_yr, aes(x = WY, y = TotalET, fill = WY)) +
#   geom_bar(stat = "identity") + scale_fill_viridis_d() + 
#   labs(x="Water Year, 2017-2024", y = "ET Total (mm)", fill = "Water Year")
...
q_sums_yr <- q_avgs %>%
  group_by(WaterYear) %>%
  summarize(TotalQ = sum(q_avgs_cms, na.rm=TRUE),
            .groups = 'drop') # Drop grouping for cleaner output

# Keep only WY 2017 going forward...
q_sums_yr <- as.data.frame(q_sums_yr[c(25:32),])

q_sums_yr$WY <- as.factor(q_sums_yr$WaterYear)

# wy_q <- ggplot(q_sums_yr, aes(x = WY, y = TotalQ, fill = WY)) +
#   geom_bar(stat = "identity") + scale_fill_viridis_d() + 
#   labs(x="Water Year, 2017-2024", y = "Q Total (cms)", fill = "Water Year")


#grid.arrange(wy_p,wy_et,wy_q, nrow=3)


# Stack df's to make one graph for water year stats

prec_sums_yr$Variable <- "Precipitation"
colnames(prec_sums_yr) <- c("WaterYear","AnnualSum","WY","Variable")

et_sums_yr$Variable <- "Evapotranspiration"
colnames(et_sums_yr) <- c("WaterYear","AnnualSum","WY","Variable")

q_sums_yr$Variable <- "Discharge"
colnames(q_sums_yr) <- c("WaterYear","AnnualSum","WY","Variable")

stat_sums_yr <- rbind.data.frame(prec_sums_yr, et_sums_yr, q_sums_yr)

stat_sums_yr$Variable <- as.factor(stat_sums_yr$Variable)


ggplot(stat_sums_yr, aes(x = WY, y = AnnualSum, fill = Variable)) +
  geom_col(position = "dodge")  + scale_fill_viridis_d() +
      labs(x="Water Year, 2017-2024", y = "Annual Total", fill = "Variable")
...
# Bird Farm Transects for population average NDVI (inserted on 11/16/2025)
...
# Create line shapefile across field where well B1 (fallow field) is located in geojson.io
# Import that feature...
pop_line <- st_read("C:/Users/19139/Desktop/NSF_Hypoth2_repeat/from_Geojsonio/pop_transect/POLYLINE.shp")
pop_line_prj <- st_transform(pop_line, crs = crs(new.crs))

# Generate points along the line
# Option A: Specify the number of points (n)
# You can also use st_sample for random sampling over the entire MULTILINESTRING
pop_pnts <- st_sample(pop_line_prj, size = 100, type = "random")
pop_pnts_multi <- as.data.frame(do.call(rbind, pop_pnts))
colnames(pop_pnts_multi) <- c("easting", "northing")
pop_pnts_multi <- st_as_sf(pop_pnts_multi, coords = c("easting", "northing"), crs = crs(new.crs))


# Convert back to lon-lat and make it look look other vegetation sample df's
pop_pnts_multi <- st_transform(pop_pnts_multi, crs = crs(pop_line))
pop_pnts_mat <- st_coordinates(pop_pnts_multi)

rf.pp.b4 <- as.data.frame(pop_pnts_mat[,c(1,2)])
colnames(rf.pp.b4) <- c("lon","lat")

rf.pp.b4$gw_well_id <- 8
...

{
  ...
  # Makes df: "pp.b4.df.rep.ps2", keep repetitions in observations
  ...
  
  {
    ...
    for(i in 1:nrow(rf.pp.b4)){
      
      a.pnt <- cbind(rf.pp.b4[i,1],rf.pp.b4[i,2]) |> vect(crs="+proj=longlat")
    ...
    pp.b4.mat.rep.ps2 <- matrix(0, nrow=nrow(pp.rf.lst.ps2[[1]]), ncol = (length(pp.rf.lst.ps2)+1))
    
    for (i in 1:length(pp.rf.lst.ps2)){
      
      a <- as.matrix(pp.rf.lst.ps2[[i]]$value, ncol=1)
      pp.b4.mat.rep.ps2[,i] <- a
      
      if(i == length(pp.rf.lst.ps2)) {
        
        b <- as.matrix(pp.rf.lst.ps2[[i]]$datez.char, ncol=1)
        pp.b4.mat.rep.ps2[,i+1] <- b
    ...
    pp.b4.df.rep.ps2 <- as.data.frame(pp.b4.mat.rep.ps2)
    pp.b4.df.rep.ps2$it <- "ps2"
  ...
  # Makes df: "pp.b4.df.rep.psbsd", keep repetitions in observations
  ...
  
  {
    ...
    for(i in 1:nrow(rf.pp.b4)){
      ...
      pp.rf.lst.psbsd[[i]] <- c1.df.ndvi.6
    ...
    pp.b4.mat.rep.psbsd <- matrix(0, nrow=nrow(pp.rf.lst.psbsd[[1]]), ncol = (length(pp.rf.lst.psbsd)+1))
    
    for (i in 1:length(pp.rf.lst.psbsd)){
      
      a <- as.matrix(pp.rf.lst.psbsd[[i]]$value, ncol=1)
      pp.b4.mat.rep.psbsd[,i] <- a
      
      if(i == length(pp.rf.lst.psbsd)) {
        
        b <- as.matrix(pp.rf.lst.psbsd[[i]]$datez.char, ncol=1)
        pp.b4.mat.rep.psbsd[,i+1] <- b
    ...
    pp.b4.df.rep.psbsd <- as.data.frame(pp.b4.mat.rep.psbsd)
    pp.b4.df.rep.psbsd$it <- "psbsd"
  ...
  # Makes df: "pp.b4.df.rep.ps2sd", keep repetitions in observations
  ...
  
  {
    ...
    for(i in 1:nrow(rf.pp.b4)){
      ...
      pp.rf.lst.ps2sd[[i]] <- c1.df.ndvi.6
    ...
    pp.b4.mat.rep.ps2sd <- matrix(0, nrow=nrow(pp.rf.lst.ps2sd[[1]]), ncol = (length(pp.rf.lst.ps2sd)+1))
    
    for (i in 1:length(pp.rf.lst.ps2sd)){
      
      a <- as.matrix(pp.rf.lst.ps2sd[[i]]$value, ncol=1)
      pp.b4.mat.rep.ps2sd[,i] <- a
      
      if(i == length(pp.rf.lst.ps2sd)) {
        
        b <- as.matrix(pp.rf.lst.ps2sd[[i]]$datez.char, ncol=1)
        pp.b4.mat.rep.ps2sd[,i+1] <- b
    ...
    pp.b4.df.rep.ps2sd <- as.data.frame(pp.b4.mat.rep.ps2sd)
    pp.b4.df.rep.ps2sd$it <- "ps2sd"
...
pp_b4_df_rep_all <- rbind.data.frame(pp.b4.df.rep.ps2,pp.b4.df.rep.psbsd,pp.b4.df.rep.ps2sd)
...
# q.data, met.data
  ...
  write.csv(pp_b4_df_rep_all, 
            "C:/Users/19139/Desktop/NSF_Hypoth2_repeat/Results_ArkansasR_09212025/pp_b4_df_rep_all.csv")
