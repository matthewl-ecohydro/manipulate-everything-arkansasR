###
### 11/11/2025
### Use output from dynamic harmonic regression 
...
y_dy_raw <- read.csv("y_dy.csv")  
y_wk_raw <- read.csv("y_wk.csv") 
y_mo_raw <- read.csv("y_mo.csv") 

# Each pfg has 4 columns (ndvi_std, pp_mean, resid, datez)

veg.name <- c("Populus deltoides", "Mixed Forbs", "Narrowleaf Willow", "Tamarix ramnossima",
              "Russian Olive", "Prairie Grass", "Fallow", "Population Avg.")

# all_raw_y <- list(pf_c1_df_rep_all, pg_c4_df_rep_all, wl_b3_df_rep_all, tm_a2_df_rep_all,
#                   ro_a1_df_rep_all,  pg_a3_df_rep_all, fl_b1_df_rep_all, pp_b4_df_rep_all)


# Put the csvs back into lists

y_dy_lst <- list()

num_cols <- ncol(y_dy_raw)

for(i in 1:length(y_dy_raw)){
  
  end_col <- min(i + 3, num_cols) # Ensure we don't go past the last column
  sub_df <- y_dy_raw[, i:end_col, drop = FALSE] # Use drop=FALSE to maintain dataframe structure
  
  y_dy_lst[[i]] <- sub_df
  
}

y_dy_lst <- y_dy_lst[c(1,5,9,13,17,21,25,29)]


y_wk_lst <- list()

num_cols <- ncol(y_wk_raw)

for(i in 1:length(y_wk_raw)){
  
  end_col <- min(i + 3, num_cols) # Ensure we don't go past the last column
  sub_df <- y_wk_raw[, i:end_col, drop = FALSE] # Use drop=FALSE to maintain dataframe structure
  
  y_wk_lst[[i]] <- sub_df
  
}

y_wk_lst <- y_wk_lst[c(1,5,9,13,17,21,25,29)]



y_mo_lst <- list()

num_cols <- ncol(y_mo_raw)

for(i in 1:length(y_mo_raw)){
  
  end_col <- min(i + 3, num_cols) # Ensure we don't go past the last column
  sub_df <- y_mo_raw[, i:end_col, drop = FALSE] # Use drop=FALSE to maintain dataframe structure
  
  y_mo_lst[[i]] <- sub_df
  
}

y_mo_lst <- y_mo_lst[c(1,5,9,13,17,21,25,29)]
...

{
  ...
  b4_phd <- tranB_phd_avg[,c(2,14,16)]
  b4_phd$B4_Temp_C <- (b4_phd$B4_Temp_F - 32) * (5/9)
  b4_phd <- b4_phd[,c(1,3:4)] # only keep dtgw, gw_temp for now
  
  b4_post <- tranB_postdoc_avg[,c(2,14,16)] # only keep dtgw, gw_temp for now
  
  b4 <- rbind.data.frame(b4_phd,b4_post)
  colnames(b4)[colnames(b4) == "datez"] <- "datez.int" 
  b4$datez <- as.Date(b4$datez.int, origin = "1970-01-01")
  
  # combine with date vector to create 3288 rows, with missing values
  b4_full <- full_join(b4, date_df, by = "datez") %>%
    complete(datez = seq(min(.$datez), max(.$datez), by = "1 day"))
  
  # I only need data from 2016-01-01 to 2024-12-31
  
  b4_full <- b4_full[b4_full$datez < "2025-01-01", ]
...
gw_lst <- list(c1_full, c4_full, b3_full, a2_full, a1_full, a3_full, b1_full, b4_full)
...
for(i in 1:length(mo_std_x)){
  ...
  y_mo_lstz <- y_mo_lst[[i]]
  
  y_mo_lstz <- y_mo_lstz %>%
    rename_with(~ "datez", contains("datez"))
  
  y_mo_lstz$datez <- as.Date(y_mo_lstz$datez, format = "%Y-%m-%d")
  
  y_x_mo_avgs <- right_join(x=x_mo_avgs, y=y_mo_lstz, by = "datez")
  ...
  y_x_mo_avgs <- y_x_mo_avgs[!is.na(y_x_mo_avgs$resid), ]
...
for(i in 1:length(wk_std_x)){
  ...
  y_wk_lstz <- y_wk_lst[[i]]
  
  y_wk_lstz <- y_wk_lstz %>%
    rename_with(~ "datez", contains("datez"))
  
  y_wk_lstz$datez <- as.Date(y_wk_lstz$datez, format = "%Y-%m-%d")
  
  y_wk_lstz$wk_year <- strftime(y_wk_lstz$datez, format = "%Y-%V")

  
  y_x_wk_avgs <- right_join(x=x_wk_avgs, y=y_wk_lstz, by = "wk_year")
  ...
  y_x_wk_avgs <- y_x_wk_avgs[!is.na(y_x_wk_avgs$resid), ]
...
for(i in 1:length(dy_std_x)){
  ...
  y_dy_lstz <- y_dy_lst[[i]]
  
  y_dy_lstz <- y_dy_lstz %>%
    rename_with(~ "datez", contains("datez"))
  
  y_dy_lstz$datez <- as.Date(y_dy_lstz$datez, format = "%Y-%m-%d")
  
  
  
  y_x_dy_avgs <- right_join(x=x_dy_avgs, y=y_dy_lstz, by = "datez")
  ...
  y_x_dy_avgs <- y_x_dy_avgs[!is.na(y_x_dy_avgs$resid), ]
...
namez_covars <- c("Avg.Temp_deg.C","Liquid.Precip_mm","ASCE.ETr_mm","Solar.Rad_W.m.2",
                  "_DTGW_m","_Temp_C","swT_m_gwT","q_cms", "gage_ht_m",
                  "Month_l0","season_l0")
...
for(i in 1:length(mo_covs_x)){
  ...
  y_df <- y_df %>%
    rename_with(~ "resid", contains("resid"))
...
for(i in 1:length(y_x_mo_fin)){
  ...
  #split_mo <- initial_time_split(y_x_mo_fin[[i]], prop = seedzz[e])
  split_mo <- initial_time_split(y_x_mo_fin[[i]], prop = seedzz[e], lag = 4)
  #split_mo <- initial_split(data = y_x_mo_fin[[i]], prop = seedzz[e])
  ...
  # number of features
  n_features <- length(setdiff(names(my_train), "resid" ))
  ...
  fit_pred_vals$Observed <- my_test$resid
...
# oos_stor[[e]] <- ggplot(obsVspred.df.mo, aes(x = Observed, y = Predicted, color = PFG)) +
# geom_point() +
...
for(e in 1:length(seedzz)){
  ...
  for(i in 1:length(y_x_mo_fin2)){
    ...
    #split_mo <- initial_time_split(y_x_mo_fin[[i]], prop = seedzz[e])
    split_mo <- initial_time_split(y_x_mo_fin2[[i]], prop = seedzz[e], lag = 4)
    #split_mo <- initial_split(data = y_x_mo_fin[[i]], prop = seedzz[e])
    ...
    #fit_pred_vals$Month <- my_test$Month_l0 # Need to turn off because not every dataset will have month as top ten covariate
    #colnames(fit_pred_vals) <- c("Predicted","Observed","Month")
  ...
  oos_stor_mo[[e]] <- ggplot(obsVspred.df.mo, aes(x = Observed, y = Predicted, color = PFG)) +
    geom_point() +
    geom_smooth(method = "lm", se = FALSE) +
    geom_text(data = r_squared_values,
              aes(x = max(obsVspred.df.mo$Observed) * 0.8,
  ...
  # Example function to extract legend
  ...
  shared_legend <- get_legend(oos_stor_mo[[1]])
...
#grid.arrange(oos_stor[[1]], ncol=1)
...
for(i in 1:length(y_x_wk_fin)){
  ...
  split_mo <- initial_time_split(y_x_wk_fin[[i]], prop = seedzz[e], lag = 4)
...
# oos_stor[[e]] <- ggplot(obsVspred.df.wk, aes(x = Observed, y = Predicted, color = PFG)) +
#   geom_point() +
#   geom_smooth(method = "lm", se = FALSE) +
#   geom_text(data = r_squared_values,
...
for(e in 1:length(seedzz)){
  ...
  for(i in 1:length(y_x_wk_fin2)){
    ...
    split_mo <- initial_time_split(y_x_wk_fin2[[i]], prop = seedzz[e], lag = 4)
  ...
  oos_stor_wk[[e]] <- ggplot(obsVspred.df.wk, aes(x = Observed, y = Predicted, color = PFG)) +
    geom_point() +
    geom_smooth(method = "lm", se = FALSE) +
    geom_text(data = r_squared_values,
              aes(x = max(obsVspred.df.wk$Observed) * 0.8,
...
for(i in 1:length(y_x_dy_fin)){
  ...
  split_mo <- initial_time_split(y_x_dy_fin[[i]], prop = seedzz[e], lag = 4)
...
# oos_stor[[e]] <- ggplot(obsVspred.df.dy, aes(x = Observed, y = Predicted, color = PFG)) +
#   geom_point() +
#   geom_smooth(method = "lm", se = FALSE) +
#   geom_text(data = r_squared_values,
...
for(e in 1:length(seedzz)){
  ...
  for(i in 1:length(y_x_dy_fin2)){
    ...
    #split_mo <- initial_time_split(y_x_dy_fin[[i]], prop = 4/5, lag = 1)
    split_mo <- initial_time_split(y_x_dy_fin2[[i]], prop = seedzz[e], lag = 4)
    #split_mo <- initial_split(data = y_x_dy_fin[[i]], prop = 0.75)
  ...
  oos_stor_dy[[e]] <- ggplot(obsVspred.df.dy, aes(x = Observed, y = Predicted, color = PFG)) +
    geom_point() +
    geom_smooth(method = "lm", se = FALSE) +
    geom_text(data = r_squared_values,
              aes(x = max(obsVspred.df.dy$Observed) * 0.8,
...
grid.arrange(
  oos_stor_mo[[1]], oos_stor_wk[[1]], oos_stor_dy[[1]],
  shared_legend,
  ncol = 3, nrow = 2,
  layout_matrix = rbind(c(1, 2, 3), c(4, 4, 4)), # Plots in row 1, legend in row 2 spanning all columns
  heights = c(4, 1) # Adjust relative heights of plot row and legend row
)


# How to plot the importance features by name/count per usage among each PFG?

imp_ft2 <- do.call(rbind, import.lst.mo)

{

# Just select top 5 variables...

imp_ft2 <- imp_ft2[, c(1:5)]

# Convert the matrix to a single column data frame
imp_ft2_df <- data.frame(Variablez = as.vector(t(imp_ft2)))

imp_ft2_df$PFG <- as.factor(rep(veg.name,each=5))

# Remove the word "sample" from the 'text_column'
strings_to_remove <- c("C1_","C4_","A1_","B3_","A2_","A3_","B1_","B4_")


# Create a single regex pattern using | (OR)
regex_pattern <- paste(strings_to_remove, collapse = "|")

# Replace in specific columns
imp_ft2_df2 <- imp_ft2_df %>%
  mutate(
    Feature = str_replace_all(Variablez, regex_pattern, "")
  )

imp_ft2_df2$Feature <- as.factor(imp_ft2_df2$Feature)

}

p.mo <- ggplot(imp_ft2_df2, aes(x = reorder(Feature, Feature, function(x) -length(x)), fill = PFG)) +
  geom_bar() +
  labs(x = "Feature", y = "Count") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) + # Optional: place legend at the bottom
  guides(fill = guide_legend(nrow = 1)) # Arrange legend items in a single row 


shared_legend <- get_legend(p.mo)


p.mo <- ggplot(imp_ft2_df2, aes(x = reorder(Feature, Feature, function(x) -length(x)), fill = PFG)) +
  geom_bar() +
  labs(x = "Feature", y = "Count", title = "(a)") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), legend.position = "none") 



imp_ft2 <- do.call(rbind, import.lst.wk)
...
p.wk <- ggplot(imp_ft2_df2, aes(x = reorder(Feature, Feature, function(x) -length(x)), fill = PFG)) +
  geom_bar() +
  labs(x = "Feature", y = "Count", title = "(b)") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), legend.position = "none") 



imp_ft2 <- do.call(rbind, import.lst.dy)
...
p.dy <- ggplot(imp_ft2_df2, aes(x = reorder(Feature, Feature, function(x) -length(x)), fill = PFG)) +
  geom_bar() +
  labs(x = "Feature", y = "Count", title = "(c)") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), legend.position = "none") 
...
# Split up data so I only have GW time periods to examine...

# The time series of the groundwater data
# is the most sparse of all covariates (5-14-18 to 10-17-20, 10-25-23 to 6-26-25)
...
for(i in 1:length(wk_std_x)){
  ...
  #Drop all rows of data where GW is NA
  
  y_x_wk_avgs <- y_x_wk_avgs[complete.cases(y_x_wk_avgs), ]
...
for(i in 1:length(dy_std_x)){
  ...
  y_x_dy_avgs <- y_x_dy_avgs[complete.cases(y_x_dy_avgs), ]
...
# Arrange the plots and the shared legend
grid.arrange(
  oos_stor_wk[[1]], oos_stor_dy[[1]],
  shared_legend,
  ncol = 2, nrow = 2,
  layout_matrix = rbind(c(1, 2), c(4, 4)), # Plots in row 1, legend in row 2 spanning all columns
  heights = c(4, 1) # Adjust relative heights of plot row and legend row
)
...
# Split up data into wet and dry period years...only weekly and daily
...
datez_wk_cov_xdry <- list()
datez_wk_cov_xwet <- list()
...
for(i in 1:length(wk_std_x)){
  ...
  y_x_wk_avgs_dry1 <- subset(y_x_wk_avgs, format(datez, "%Y") == "2018")
  y_x_wk_avgs_dry2 <- subset(y_x_wk_avgs, format(datez, "%Y") == "2020")
  y_x_wk_avgs_dry3 <- subset(y_x_wk_avgs, format(datez, "%Y") == "2024")
  
  y_x_wk_avgs_wet1 <- subset(y_x_wk_avgs, format(datez, "%Y") == "2017")
  y_x_wk_avgs_wet2 <- subset(y_x_wk_avgs, format(datez, "%Y") == "2019")
  y_x_wk_avgs_wet3 <- subset(y_x_wk_avgs, format(datez, "%Y") == "2023")
  
  y_x_wk_avgs_dry <- rbind(y_x_wk_avgs_dry1, y_x_wk_avgs_dry2, y_x_wk_avgs_dry3)
  y_x_wk_avgs_wet <- rbind(y_x_wk_avgs_wet1, y_x_wk_avgs_wet2, y_x_wk_avgs_wet3)
  
  #Keep/store the dates column
  datez_wk_cov_xdry[[i]] <- y_x_wk_avgs_dry[,c("datez")]
  datez_wk_cov_xwet[[i]] <- y_x_wk_avgs_wet[,c("datez")]
  
  # y_x_wk_avgs_cln <- y_x_wk_avgs[,-c(1,11,12,22,32,34)]
  
  wk_covs_x_dry[[i]] <- y_x_wk_avgs_dry
  wk_covs_x_wet[[i]] <- y_x_wk_avgs_wet
...
datez_dy_cov_xdry <- list()
datez_dy_cov_xwet <- list()
...
for(i in 1:length(dy_std_x)){
  ...
  y_x_dy_avgs_dry1 <- subset(y_x_dy_avgs, format(datez, "%Y") == "2018")
  y_x_dy_avgs_dry2 <- subset(y_x_dy_avgs, format(datez, "%Y") == "2020")
  y_x_dy_avgs_dry3 <- subset(y_x_dy_avgs, format(datez, "%Y") == "2024")
  
  y_x_dy_avgs_wet1 <- subset(y_x_dy_avgs, format(datez, "%Y") == "2017")
  y_x_dy_avgs_wet2 <- subset(y_x_dy_avgs, format(datez, "%Y") == "2019")
  y_x_dy_avgs_wet3 <- subset(y_x_dy_avgs, format(datez, "%Y") == "2023")
  
  y_x_dy_avgs_dry <- rbind(y_x_dy_avgs_dry1, y_x_dy_avgs_dry2, y_x_dy_avgs_dry3)
  y_x_dy_avgs_wet <- rbind(y_x_dy_avgs_wet1, y_x_dy_avgs_wet2, y_x_dy_avgs_wet3)
  
  #Keep/store the dates column
  datez_dy_cov_xdry[[i]] <- y_x_dy_avgs_dry[,c("datez")]
  datez_dy_cov_xwet[[i]] <- y_x_dy_avgs_wet[,c("datez")]
  
  # y_x_dy_avgs_cln <- y_x_dy_avgs[,-c(1,11,12,22,32,34)]
  
  dy_covs_x_dry[[i]] <- y_x_dy_avgs_dry
  dy_covs_x_wet[[i]] <- y_x_dy_avgs_wet
...
for(i in 1:length(wk_covs_x_dry)){
...
for(i in 1:length(wk_covs_x_wet)){
...
for(i in 1:length(dy_covs_x_wet)){
...
for(i in 1:length(dy_covs_x_dry)){
...
for(e in 1:length(seedzz)){
  ...
  for(i in 1:length(y_x_wk_fin_d)){
    
    #split_mo <- initial_time_split(y_x_wk_fin[[i]], prop = 4/5, lag = 1)
    split_mo <- initial_time_split(y_x_wk_fin_d[[i]], prop = seedzz[e], lag = 4)
    #split_mo <- initial_split(data = y_x_wk_fin[[i]], prop = 0.75)
...
for(e in 1:length(seedzz)){
  ...
  impurz.lst.wk <- list()
  permz.lst.wk <- list()
  import.lst.wk_d <- list()
  
  for(i in 1:length(y_x_wk_fin2_d)){
    
    #split_mo <- initial_time_split(y_x_wk_fin2[[i]], prop = 4/5, lag = 1)
    split_mo <- initial_time_split(y_x_wk_fin2_d[[i]], prop = seedzz[e], lag = 4)
    #split_mo <- initial_split(data = y_x_wk_fin2[[i]], prop = 0.75)
    ...
    import.lst.wk_d[[i]] <- top_10_features
  ...
  oos_stor_wk_d[[e]] <- ggplot(obsVspred.df.mo, aes(x = Observed, y = Predicted, color = PFG)) +
    geom_point() +
    geom_smooth(method = "lm", se = FALSE) +
    geom_text(data = r_squared_values,
              aes(x = max(obsVspred.df.mo$Observed) * 0.8,
  ...
  # Extract the legend from one of the plots
  shared_legend <- get_legend(oos_stor_wk_d[[1]])
...
for(e in 1:length(seedzz)){
  ...
  for(i in 1:length(y_x_wk_fin_w)){
    
    #split_mo <- initial_time_split(y_x_wk_fin[[i]], prop = 4/5, lag = 1)
    split_mo <- initial_time_split(y_x_wk_fin_w[[i]], prop = seedzz[e], lag = 4)
    #split_mo <- initial_split(data = y_x_wk_fin[[i]], prop = 0.75)
...
for(e in 1:length(seedzz)){
  ...
  impurz.lst.wk <- list()
  permz.lst.wk <- list()
  import.lst.wk_w <- list()
  
  for(i in 1:length(y_x_wk_fin2_w)){
    
    #split_mo <- initial_time_split(y_x_wk_fin2[[i]], prop = 4/5, lag = 1)
    split_mo <- initial_time_split(y_x_wk_fin2_w[[i]], prop = seedzz[e], lag = 4)
    #split_mo <- initial_split(data = y_x_wk_fin2[[i]], prop = 0.75)
    ...
    import.lst.wk_w[[i]] <- top_10_features
  ...
  oos_stor_wk_w[[e]] <- ggplot(obsVspred.df.wk, aes(x = Observed, y = Predicted, color = PFG)) +
    geom_point() +
    geom_smooth(method = "lm", se = FALSE) +
    geom_text(data = r_squared_values,
              aes(x = max(obsVspred.df.wk$Observed) * 0.8,
...
#grid.arrange(oos_stor_wk_d[[1]], oos_stor_wk_w[[1]], ncol=2)



# Arrange the plots and the shared legend
grid.arrange(
  oos_stor_wk_d[[1]], oos_stor_wk_w[[1]],
  shared_legend,
  ncol = 2, nrow = 2,
  layout_matrix = rbind(c(1, 2), c(4, 4)), # Plots in row 1, legend in row 2 spanning all columns
  heights = c(4, 1) # Adjust relative heights of plot row and legend row
...
imp_ft2 <- do.call(rbind, import.lst.wk_d)
...
p.wk.d <- ggplot(imp_ft2_df2, aes(x = reorder(Feature, Feature, function(x) -length(x)), fill = PFG)) +
  geom_bar() +
  labs(x = "Feature", y = "Count") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) + # Optional: place legend at the bottom
  guides(fill = guide_legend(nrow = 1)) # Arrange legend items in a single row 
...
shared_legend <- get_legend(p.wk.d)
...
imp_ft2 <- do.call(rbind, import.lst.wk_w)
...
p.wk.w <- ggplot(imp_ft2_df2, aes(x = reorder(Feature, Feature, function(x) -length(x)), fill = PFG)) +
  geom_bar() +
  labs(x = "Feature", y = "Count", title = "(a)") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), legend.position = "none")
...
for(e in 1:length(seedzz)){
  ...
  for(i in 1:length(y_x_dy_fin_d)){
    
    #split_mo <- initial_time_split(y_x_dy_fin[[i]], prop = 4/5, lag = 1)
    split_mo <- initial_time_split(y_x_dy_fin_d[[i]], prop = seedzz[e], lag = 4)
    #split_mo <- initial_split(data = y_x_dy_fin[[i]], prop = 0.75)
...
for(e in 1:length(seedzz)){
  ...
  impurz.lst.dy <- list()
  permz.lst.dy <- list()
  import.lst.dy_d <- list()
  
  for(i in 1:length(y_x_dy_fin2_d)){
    
    #split_mo <- initial_time_split(y_x_dy_fin[[i]], prop = 4/5, lag = 1)
    split_mo <- initial_time_split(y_x_dy_fin2_d[[i]], prop = seedzz[e], lag = 4)
    #split_mo <- initial_split(data = y_x_dy_fin[[i]], prop = 0.75)
    ...
    import.lst.dy_d[[i]] <- top_10_features
  ...
  oos_stor_dy_d[[e]] <- ggplot(obsVspred.df.dy, aes(x = Observed, y = Predicted, color = PFG)) +
    geom_point() +
    geom_smooth(method = "lm", se = FALSE) +
    geom_text(data = r_squared_values,
              aes(x = max(obsVspred.df.dy$Observed) * 0.8,
  ...
  # Extract the legend from one of the plots
  shared_legend <- get_legend(oos_stor_dy_d[[1]])
...
for(e in 1:length(seedzz)){
  ...
  for(i in 1:length(y_x_dy_fin_w)){
    
    #split_mo <- initial_time_split(y_x_dy_fin[[i]], prop = 4/5, lag = 1)
    split_mo <- initial_time_split(y_x_dy_fin_w[[i]], prop = seedzz[e], lag = 4)
    #split_mo <- initial_split(data = y_x_dy_fin[[i]], prop = 0.75)
...
for(e in 1:length(seedzz)){
  ...
  impurz.lst.dy <- list()
  permz.lst.dy <- list()
  import.lst.dy_w <- list()
  
  for(i in 1:length(y_x_dy_fin2_w)){
    
    #split_mo <- initial_time_split(y_x_dy_fin[[i]], prop = 4/5, lag = 1)
    split_mo <- initial_time_split(y_x_dy_fin2_w[[i]], prop = seedzz[e], lag = 4)
    #split_mo <- initial_split(data = y_x_dy_fin[[i]], prop = 0.75)
    ...
    import.lst.dy_w[[i]] <- top_10_features
  ...
  oos_stor_dy_w[[e]] <- ggplot(obsVspred.df.dy, aes(x = Observed, y = Predicted, color = PFG)) +
    geom_point() +
    geom_smooth(method = "lm", se = FALSE) +
    geom_text(data = r_squared_values,
              aes(x = max(obsVspred.df.dy$Observed) * 0.8,
...
# Arrange the plots and the shared legend
grid.arrange(
  oos_stor_dy_d[[1]], oos_stor_dy_w[[1]],
  shared_legend,
  ncol = 2, nrow = 2,
  layout_matrix = rbind(c(1, 2), c(4, 4)), # Plots in row 1, legend in row 2 spanning all columns
  heights = c(4, 1) # Adjust relative heights of plot row and legend row
...
imp_ft2 <- do.call(rbind, import.lst.dy_d)
...
p.dy.d <- ggplot(imp_ft2_df2, aes(x = reorder(Feature, Feature, function(x) -length(x)), fill = PFG)) +
  geom_bar() +
  labs(x = "Feature", y = "Count") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) + # Optional: place legend at the bottom
  guides(fill = guide_legend(nrow = 1)) # Arrange legend items in a single row 
...
shared_legend <- get_legend(p.dy.d)
...
imp_ft2 <- do.call(rbind, import.lst.dy_w)
...
p.dy.w <- ggplot(imp_ft2_df2, aes(x = reorder(Feature, Feature, function(x) -length(x)), fill = PFG)) +
  geom_bar() +
  labs(x = "Feature", y = "Count", title = "(a)") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), legend.position = "none")
...
#namez_lagz <- c("_l0","_l1","_l_1_4avg", "_l_5_8avg")
...
for(i in 1:length(y_x_mo_fin2)){
  ...
  importance_scores <- vi(rf_impurity, method = "model")
  sorted_importance <- importance_scores[order(importance_scores$Importance, decreasing = TRUE), ]
  top_5_features <- sorted_importance$Variable[1:5]
  ...
  import.lst.mo2[[i]] <- top_5_features
...
imp_ft2 <- do.call(rbind, import.lst.mo2)

#imp_ft2 <- as.data.frame(table(imp_ft2))
...
# Remove the word "sample" from the 'text_column'
strings_to_remove <- c("C1_","C4_","A1_","B3_","A2_","A3_","B1_")
...
ggplot(imp_ft2_df2, aes(x = reorder(Feature, Feature, function(x) -length(x)), fill = PFG)) +
  geom_bar() +
  labs(x = "Feature", y = "Count") +
...
for(i in 1:length(y_x_wk_fin2)){
  ...
  import.lst.wk2[[i]] <- top_5_features
...
imp_ft2 <- do.call(rbind, import.lst.wk2)
...
for(i in 1:length(y_x_dy_fin2)){
  ...
  import.lst.dy2[[i]] <- top_5_features
...
imp_ft2 <- do.call(rbind, import.lst.dy2)
